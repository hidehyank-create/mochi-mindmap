...
(function(){
  ...
  test("F27M-01 実根本Rポリゴンを共通開口判定に使用",()=>{reset([N("a",300,300,80,2),N("b",650,300,70,1)]);const l={id:"ab",a:"a",b:"b",control:null,seq:1};links.push(l);const g=f23RootGeom(l,true);A(g,"根本R geometryなし");const p=g.top[0];A(M.ownOpeningAt?.(nodes[0],p),"根本R接点を開口として認識しない")});
  ...
  test("F27M-04 接着候補も根本R開口を拒否",()=>{reset([N("target",500,300,80,2),N("hose",800,300,60,1),N("moved",410,300,60,3)]);links.push({id:"th",a:"target",b:"hose",control:null,seq:1});A(M.pairBlockedAtOpening?.({movedId:"moved",otherId:"target"}),"根本R側の接着候補を拒否しない")});
