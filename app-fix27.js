...
(function installFix27(){
  ...
  function attachedPairPivot(n){const seams=directAttachmentsOf(n.id),ids=componentIds(n.id);if(seams.length!==1||ids.length!==2)return null;const m=seams[0],pivotId=m.a===n.id?m.b:m.a,pivot=nodeById(pivotId);return pivot?{pivot,pivotId,ids}:null}
  function rigidPivotPoint(pivot,radius,p){const u=unit(p.x-pivot.x,p.y-pivot.y);return{x:pivot.x+u.x*radius,y:pivot.y+u.y*radius}}
  function beginRigidPivot(evt,n,p){const pair=attachedPairPivot(n);if(!pair)return false;selected={type:"node",id:n.id};bringComponentFront([n.id]);gesture={pointerId:evt.pointerId,mode:"pivot",nodeId:n.id,pivotId:pair.pivotId,compIds:[n.id],start:p,starts:{[n.id]:{x:n.x,y:n.y}},points:[p],pivotRadius:dist(n.x,n.y,pair.pivot.x,pair.pivot.y),contactPair:null,inputType:evt.pointerType||"pen",f27RigidPivot:true};cap(evt.pointerId);statusText("接着したまま回転：長さ固定");scheduleMotionRender();return true}
  function rigidGroupMove(g,p){const before={};for(const id of g.compIds){const n=nodeById(id);if(n)before[id]={x:n.x,y:n.y,r:n.r}}const v0=T26.violation(g.compIds),dx=p.x-g.start.x,dy=p.y-g.start.y,hit=collisionAlongMove(g.compIds,g.starts,dx,dy,g.crossMode),safeT=hit.pair?Math.max(0,hit.t-.002):1;setMovePositions(g,dx,dy,safeT);cleanupCrossPairs();if(T26.constrain(g.compIds,before,v0)){statusText("2個グループ移動：紐との隙間を保持")}else if(hit.pair&&!g.crossMode){setContact(g,hit.pair);statusText("2個グループ移動：接触中")}else{setContact(g,null);statusText("2個グループ移動：一体平行移動")}g.last=p;scheduleMotionRender()}
  ...
  penDown=function(evt,allowTouch=false){
    ...
    if(resizeArmedId&&(!n||n.id!==resizeArmedId)){resizeArmedId=null;renderUI()}
    ...
    if(n&&attachedPairPivot(n)){evt.preventDefault();if(beginRigidPivot(evt,n,p))return}
    const r=prevPenDown(evt,allowTouch);suppressLegacyNodeResize(gesture);return r
  };
  penMove=function(evt){
    if(gesture?.mode==="f27GroupPending"&&evt.pointerId===gesture.pointerId){evt.preventDefault();const g=gesture,p=eventToWorld(evt);g.last=p;const th=(g.inputType==="pen"?PEN_MOVE_THRESHOLD:MOVE_THRESHOLD)/view.scale;if(dist(g.start.x,g.start.y,p.x,p.y)>th){if(g.holdTimer){clearTimeout(g.holdTimer);g.holdTimer=0}gesture=null;const n=nodeById(g.nodeId);if(n){beginRigidPivot(evt,n,g.start);return penMove(evt)}}return}
  ...
  svg.addEventListener("dblclick",evt=>{const p=eventToWorld(evt),n=dblTargetAt(p);if(!n)return;evt.preventDefault();evt.stopPropagation();selected={type:"node",id:n.id};resizeArmedId=n.id;gesture=null;statusText("リサイズモード：黄色／丸全体が対象（上の小丸部分は除外）");renderUI()},true);
  ...
  window.__mochiFix27Test={nearByHoseHalf,nearDiff:F27_NEAR_DIFF,resizeArmed:()=>resizeArmedId,outerRuns,hiddenCircleRuns:f25HiddenCircleRuns,workToolActive,suppressLegacyNodeResize,dblTargetAt,rigidGroupMove,attachedPairPivot,rigidPivotPoint};
