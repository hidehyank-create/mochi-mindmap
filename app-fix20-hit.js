...
nearestNode=function(x,y){const n=f20PickNode(x,y,24/view.scale);return n?{node:n,d:dist(x,y,n.x,n.y)}:null};
targetAtWorld=function(p){const n=f20PickNode(p.x,p.y,18/view.scale);if(n)return{type:"node",id:n.id};const l=nearestLink(p.x,p.y,26/view.scale);if(l)return{type:"link",id:l.link.id};return null};
...
if(window.__mochiFix20Test){window.__mochiFix20Test.hitsAt=f20HitsAt;window.__mochiFix20Test.pickNode=f20PickNode}
