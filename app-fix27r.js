...
(function installFix27R(){
  ...
  const diagnosticStyle=document.createElement("style");diagnosticStyle.id="f27tLayerColors";diagnosticStyle.textContent=`#hiddenLayer path.hidden-outline{stroke:#e32636!important}#zTopLayer path{stroke:#1769e0!important}#zTopLayer path[data-z-boundary="link-node-gap"]{stroke:#f59e0b!important}#overlapLayer path{stroke:#16a34a!important}#shadowLayer path,#linksLayer path,#mergeLayer path,#liveLayer path{stroke:transparent!important;fill:transparent!important}`;document.head.appendChild(diagnosticStyle);
  ...
  statusText=function(t){status.innerHTML=`餅マップ v0.9.4<br>BUILD ${F27R_BUILD}<br>${t}`;f22PlaceStatus()};renderAll();statusText("診断色：hidden＝赤／zTop＝青／gapマスク＝橙／overlap＝緑／その他＝紫");
